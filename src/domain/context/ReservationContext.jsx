...
export function ReservationProvider({ children }) {
  ...
  // Cargar datos desde servicio al montar
  useEffect(() => {
    async function load() {
      const data = await reservationService.getReservations()
      if (data) setReservations(data)
      else setReservations(SAMPLE_RESERVATIONS)
      setIsLoading(false)
    }
    load()
  }, [])

  // Persistir reservas usando servicio
  useEffect(() => {
    if (!isLoading) {
  ...
  /** updateReservationItems — Añade nuevos platos y reinicia estado de cocina. */
      ...
      prev.map(r => {
        if (r.id === id) {
           ...
           return { 
             ...r, 
             items: merged, 
             kitchenStatus: merged.length > 0 ? 'pending' : r.kitchenStatus, 
  ...
  /** deleteReservation — Elimina permanentemente una reserva (solo lider/admin). */
  const deleteReservation = useCallback((id) => {
    setReservations(prev => prev.filter(r => r.id !== id))
  }, [])

  /** deleteAllReservations — Elimina permanentemente TODAS las reservas (solo líder). */
