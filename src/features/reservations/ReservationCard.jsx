...
import { Edit2, CheckCircle, UserCheck, XCircle, Clock, Users, MapPin, Phone, Star, Trash2 } from 'lucide-react'
...
export default function ReservationCard({ reservation: r, onEdit, onSeat, onComplete, onCancel, canCancel, canDelete, canSeat }) {
  ...
  return (
    <article className={`${styles.card} ${styles[`card--${r.status}`]}`}>
      ...
      <div className={styles.body}>
        ...
        {/* Acciones */}
        {isActive && !showCancelInput && !showDeleteConfirm && (
        ...
        {/* Acciones en reservas no activas: solo líder puede eliminar */}
        {!isActive && canDelete && !showDeleteConfirm && (
          <div className={styles.actions}>
            {onEdit && (
              <Button variant="ghost" size="sm" icon={<Edit2 size={14} />} onClick={onEdit}>
                Editar
        ...
        {showDeleteConfirm && (
          <div className={styles.cancelBox} style={{ borderColor: 'var(--color-error)' }}>
            <p style={{ fontSize: '12px', color: 'var(--color-error)', fontWeight: 600, marginBottom: 8 }}>
              ⚠️ ¿Eliminar permanentemente la reserva #{r.id}? Esta acción no se puede deshacer.
            </p>
            <div className={styles.cancelActions}>
              <Button variant="danger" size="sm" onClick={() => { onDelete(); setShowDeleteConfirm(false) }}>
                Sí, eliminar
              </Button>
